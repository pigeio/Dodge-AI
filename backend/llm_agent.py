...
def generate_response(user_query: str) -> str:
    ...
    final_system_prompt = SYSTEM_PROMPT + f"\n\nCRITICAL SQL RULES & EXAMPLES:\n- The 'products' table uses 'product' as its ID column.\n- In item tables (sales_order_items, billing_document_items), the product ID is called 'material'.\n- NEVER use 'product_id' or 'material_id'.\n- Example 1 (Highest billing products):\n  SELECT material as product, count(*) as billing_count FROM billing_document_items GROUP BY material ORDER BY billing_count DESC LIMIT 10;\n- Example 2 (Row counts):\n  SELECT COUNT(*) FROM sales_order_headers;\n- Example 3 (Broken flows - delivered but not billed):\n  SELECT DISTINCT odi.deliveryDocument, odi.referenceSdDocument as salesOrder FROM outbound_delivery_items odi LEFT JOIN billing_document_items bdi ON odi.referenceSdDocument = bdi.referenceSdDocument WHERE bdi.billingDocument IS NULL;\n- Example 4 (Billed without delivery):\n  SELECT DISTINCT bdi.billingDocument, bdi.referenceSdDocument FROM billing_document_items bdi LEFT JOIN outbound_delivery_items odi ON bdi.referenceSdDocument = odi.referenceSdDocument WHERE odi.deliveryDocument IS NULL;\n\nDATABASE SCHEMA FOR SQL QUERIES:\n{schema_str}"
